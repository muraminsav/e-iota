  ...
  for (let color of colors) {
    for (let shape of shapes) {
      for (let number of numbers) {
        deck.push({ color: color, shape: shape, number: number, id: count });
        count++;
      }
    }
  }
...
function returnCards(cards = [], deck) {
  cards.forEach((card) => {
    deck.push(card);
  });
}
...
export { Card, Player, Game, Session, generateDeck, pickOne, returnCards };
